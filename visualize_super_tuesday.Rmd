...
pal <- colorNumeric(
  palette = "viridis",
  domain = super_tuesday_delegates$sanders_share)
leaflet()%>%
...
It's hard to say what the impact was of the coverage that inflated our expectations for Sanders. One the one hand, positive polling probably helped Sanders build enthusiasm and belief within his base. However, abrupt shift away from that positivity may do more harm than good.
...
Note that the first chart assumes (in addition to the polls being accurate) that none of Klobuchar and Buttigieg's supporters switch to either candidate, which is obviously not a good assumption given what we saw on Super Tuesday. The actual reality, as things currently stand, is even bleaker for Sanders. 

The second chart entertains the idea of Warren dropping out and enthusiastically endorsing Sanders, so effectively that all of her supporters switch to Sanders, while all Klobuchar and Buttigieg supporters in remaining states go for Biden. This obviously won't happen in totality, but it gives us some idea of how a potential endorsement would change things. This looks slightly less bleak for Sanders. A handful of states shift above the line, but the gap does grow larger in other large states (notably Florida). 
...
In a word, maybe? Even if we put aside the issue of potential realignment and just focus on the polling for Sanders and Biden, the answer may not be clear. 
...
polling_comparison<-primary_polls%>%
...
#function to create donuts, takes df with columns "variable"
create_donut<-function(df,chart_name){
  #because donut charts are actually polar charts, need to create a cumulative sum variable for the value
  df%>%mutate(ymax=cumsum(value),
         ymin=c(0,head(ymax,n=-1)))%>%
  #xmin and xmax determine the size of the hole
  ggplot(aes(fill=variable,ymin=ymin,ymax=ymax,xmin=3,xmax=5))+
  #make it a rectangular chart
  geom_rect(inherit.aes=TRUE)+
  #make chart polor
  coord_polar(theta="y")+
  #set range of x values, taken with xmin and xmax creates size of the hole
  xlim(c(0,5))+
  #remove background
  theme_void() +
  #no labels
  ...
  theme(legend.position="bottom")+
  #define the title as equal to the chart_name argument, to be passed when the function is called
  labs(title=chart_name)+
  geom_label(
    aes(x=4,y=(ymax+ymin)/2,label=comma(value),color=variable),
    label.size=0.25,
    show.legend=FALSE
  )+

  scale_color_manual(values=c(rgb_max(250,250,250),rgb_max(250,250,250),rgb_max(250,250,250)))#+
  #scale_fill_brewer(palette="Dark2")
}

grid.arrange(create_donut(polling_comparison,chart_name="Total Polls Conducted"),
             create_donut(delegates_remaining,chart_name="Delegate Count"),ncol=2)
...
```{r compare_good_polls,echo=FALSE,message=FALSE,warning=FALSE}
quality_polls<-c("A+","A","A-","A/B","B+")
...
polling_comparison<-primary_polls%>%
  ...
  filter(fte_grade%in%quality_polls)%>%
  #create indicator for whether or not state is early
  mutate(variable=ifelse(state%in%early_state,
                            "Super Tuesday or Earlier",
                            "Remaining States"))%>%
...
grid.arrange(create_donut(polling_comparison,chart_name="Number B+ or better Polls Conducted"),
             create_donut(delegates_remaining,chart_name="Delegate Count"),ncol=2)
